        ...
        private void OpenOld(byte[] data)
        {
            ...
            using (BinaryReader reader = Utilities.IO.OpenReadFile(new MemoryStream(data), true))
                while (reader.BaseStream.Position < reader.BaseStream.Length - 0x100)
                {
                    ...
                    reader.BaseStream.Position += Utilities.UtilitiesTool.Alignment(reader.BaseStream.Position, 0x40);
            ...
            if (Old)
            {
                foreach (var a in SubFiles)
                    if (a.Object is IFile file)
                    {
                        returned += 0x100;
                        returned += file.Size();
                        returned += Utilities.UtilitiesTool.Alignment(returned, 0x40);
                    }
                returned += 0x100;
            }
            else
            {
                ...
                foreach (var a in SubFiles)
                    if (a.Object is IFile file)
                    {
                        returned += 0x20 + 4;
                        int size = file.Size();
                        int align = Utilities.UtilitiesTool.Alignment(size, 0x20);
                        returned += size + align;
                    }
            }
            ...
            using (MemoryStream MS = new MemoryStream())
            {
                ...
                foreach (var a in SubFiles)
                    if (a.Object is IPersonaFile pfile)
                    {
                        ...
                        writer.Write(size);
                        writer.Write(data);
                        writer.Write(new byte[Utilities.UtilitiesTool.Alignment(MS.Position, 0x40)]);
                    }
                ...
                foreach (var a in SubFiles)
                    if (a.Object is IFile file)
                        if (a.Object is IPersonaFile pfile)
                        {
                            ...
                            writer.Write(new byte[Utilities.UtilitiesTool.Alignment(a.Name.Length, 0x20)]);
                            int size = file.Size();
                            int align = Utilities.UtilitiesTool.Alignment(size, 0x20);
