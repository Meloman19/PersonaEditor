// Original code https://github.com/Crazycatz00/nQuant
// Fork adaped for System.Windows.Media namespace
...
namespace PersonaEditorLib.Media.Imaging.Quantization
{
    struct Box
    {
    ...
    struct PaletteColorHistory
    {
        ...
        public System.Windows.Media.Color ToNormalizedColorWPF()
        {
            return (Sum != 0) ? System.Windows.Media.Color.FromArgb((byte)(Alpha / Sum), (byte)(Red / Sum), (byte)(Green / Sum), (byte)(Blue / Sum)) : System.Windows.Media.Colors.Transparent;
        }
    ...
    public class WuQuantizer : IQuantization
    {
        ...
        public bool StartQuantization(BitmapSource bitmapSource)
        {
            var colorconvert = PixelConverter.GetDataColorConverter(PixelFormatHelper.ConvertFromSystem(bitmapSource.Format));
            if (colorconvert == null)
                return false;

            int Width = bitmapSource.PixelWidth;
            int Height = bitmapSource.PixelHeight;
            int stride = Utilities.ImageHelper.GetStride(bitmapSource.Format, Width);
            int LengthData = stride * Height;
            byte[] imageData = new byte[LengthData];

            bitmapSource.CopyPixels(imageData, stride, 0);
            Color[] image = colorconvert(imageData, bitmapSource.Palette?.Colors.ToArray());
            int maxColors = Convert.ToInt32(Math.Pow(2, PixelFormat.BitsPerPixel));
        ...
        public bool StartQuantization(Color[] image)
        {
            if (image == null)
                return false;
        ...
        public static Tuple<Color[], byte[]> QuantizeImage(Color[] image, int alphaThreshold = 10, int alphaFader = 70, int maxColors = 256)
        {
            ...
            var returnedColors = BuildPalette(new System.Windows.Media.Color[maxColors], paletteHistogram);
        ...
        private static byte[] IndexedPixels(Color[] pixels, Pixel[] lookups, int alphaThreshold, int maxColors, PaletteColorHistory[] paletteHistogram)
        {
            ...
            {
                Pixel pixel = new Pixel(pixels[i].A, pixels[i].R, pixels[i].G, pixels[i].B);
        ...
        private static ColorMoment[,,,] BuildHistogram(Pixel[] sourcePixels, int alphaThreshold, int alphaFader)
        {
        ...
        {
            System.Windows.Media.Color[] lookups = new System.Windows.Media.Color[cubes.Length];
            ...
            for (int cubeIndex = 0; cubeIndex < cubes.Length; cubeIndex++)
            {
                ...
                lookups[cubeIndex] = System.Windows.Media.Color.FromArgb
